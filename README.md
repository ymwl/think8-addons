...
**v1.2.2**（2026-09-26）
- **新增伪静态 301 规范化（SEO，无破坏性变更）**：插件配置了 `rewrite` 伪静态规则后，经原生插件路由（`/addons/...`）访问的 GET/HEAD 请求会 301 重定向到对应的伪静态地址，让搜索引擎只收录规范地址，避免同一内容因两份地址被判为重复内容
- 重定向目标由 `addon_rewrite_url()` 生成，与插件内部 `addon_url()` 的输出完全一致，且仅在规则已真实注册到框架路由表时采用，不会跳转到打不开的地址；目标与当前路径一致时跳过，杜绝循环重定向
- 仅 GET/HEAD 请求跳转，POST 等写操作保持原样（避免丢失请求体）；未配置伪静态、规则未命中或尚未同步到 `route` 的插件不受影响，行为与升级前完全一致
- 实现位于 `src/addons/Route.php`：`Route::execute()` 在控制器实例化前统一处理，插件无需做任何改动
- `Version::VERSION` 同步更新为 `1.2.2`（与 tag 同步）
...
>
> 经原生插件路由地址（`/addons/...`）访问时，本扩展包会 301 重定向到对应的伪静态地址（v1.2.2+，仅 GET/HEAD），让搜索引擎只收录伪静态这一份规范地址，避免重复内容分散权重；伪静态地址直达不受影响，POST 等写操作不跳转。
